...
- [x] [Review][Patch] PromptPasskeyEnrollment uses `"webauthn"`/`"webauthn-register"` instead of passwordless types (`webauthn-passwordless`/`webauthn-register-passwordless`) — re-prompts users who already have passkeys and registers the wrong credential type (verify intended passkey model) [`PromptPasskeyEnrollment.java`:459]
